...
namespace LoginInterface
{
    public partial class AcceptPayment : Form
    {
        ...
        private void Search()
        {
            ...
            if (valid.isStudentExist(txtUsername.Texts))
            {
                ...
                SqlDataReader drd = con.DataReader
                    ($"SELECT name,ic_passport,contact_num,subject FROM student WHERE username = '{txtUsername.Texts}'");
                while (drd.Read())
                {
                    sub = ((drd[3].ToString()));
                    lblName.Text = drd[0].ToString();
                    lblCPassport.Text = drd[1].ToString();
                    lblContactNum.Text = drd[2].ToString();
                }
                if (sub.Contains('/'))
                {
                    subs = sub.Split('/').ToList();
                    if (subs.Count == 2)
                    {
                        grpSub2.Location = new System.Drawing.Point(135, 412);
                        grpSub2.Visible = true;
                        checkBox5.Text = $"{subs[0]}";
                        checkBox6.Text = $"{subs[1]}";
                    }
                    else if (subs.Count == 3)
                    {
                        ...
                        checkBox1.Text = $"{subs[0]}";
                        checkBox2.Text = $"{subs[1]}";
                        checkBox3.Text = $"{subs[2]}";
                    }
                }
                else
                {
        ...
        private void checkBox7_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                subsSelected.Remove(checkBox7.Text);
            }
        }
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            recep.UpdatePayment(txtUsername.Texts);
