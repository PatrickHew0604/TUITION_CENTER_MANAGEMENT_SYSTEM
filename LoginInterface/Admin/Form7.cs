...
namespace LoginInterface
{
    ...
    {    
        ...
        private void Search()
        {
            ...
            if ((new Validation()).isTutorExist(txtUsername.Text))
            {
                SqlDataReader dr = con.DataReader($"SELECT name,ic_number,contact_number FROM staff WHERE username = '{txtUsername.Text}' AND role = 'Tutor'");
                while (dr.Read())
                {
        ...
        private void btnRemove_Click(object sender, EventArgs e)
        {
            ...
            admin.RemoveTutor(txtUsername.Text);
            //need to change to Patrick special message box
            Notification noti = new Notification("Tutor has been deleted");
            noti.Show();
            this.Close();
        }
    }
